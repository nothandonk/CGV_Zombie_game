...
class Zombie {
  ...
  checkCollisions(nextPosition) {
    const nextBoundingBox = this.boundingBox
      .clone()
      .translate(nextPosition.sub(this.model.position));

    for (const object of this.world.objectsToCheck) {
      if (object !== this.model && object.geometry) {
        // Ignore self
  ...
  update(delta) {
    ...
    if (this.model) {
      ...
      if (
        distanceToCamera >
        stoppingDistance /* && !this.checkCollisions(nextPosition) */
      ) {
        ...
        if (
          ...
          this.currentAction !== this.animations["punching"]
        ) {
